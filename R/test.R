...
if(download) {
  ...
  expDat <- tcgaProjects %>% 
    group_by(project_id) %>% 
    do(summexp = downloadExp(.$project_id, "LRP1B")) %>%
    mutate(failed = is.null(summexp)) %>%
    filter(!failed)
  
  # Retry ones that didn't work, but without GISTIC data...
  expDat2 <- tcgaProjects %>% 
    group_by(project_id) %>% 
    filter(!(project_id %in% expDat$project_id)) %>%
    do(summexp = downloadExp(.$project_id, NULL)) %>%
    mutate(failed = is.null(summexp)) %>%
    filter(!failed)
  
  expDat <- full_join(expDat, expDat2)
  ...
  load("GDCdata/gbm.rda") 
  gbm <- data
  load("GDCdata/lgg.rda")
  lgg <- data
  rm(data)
}
